...
#define serHANDLE					( ( xComPortHandle ) 0 )
#define serNO_BLOCK					( ( TickType_t ) 0 )

/*-----------------------------------------------------------*/
#define serUART_ID					BOARD_CONSOLE_UART_ID

static DEV_UART *serUART;
...
extern void vSerial_RXCallback (void *par);
extern void vSerial_TXCallback (void *par);
...
xComPortHandle xSerialPortInitMinimal( unsigned long ulWantedBaud, unsigned portBASE_TYPE uxQueueLength )
{
	...
	if (
		...
		portENTER_CRITICAL();
		{
			serUART = uart_get_dev(serUART_ID);
			if (serUART == NULL) {
				xReturn = ( xComPortHandle ) 0;
			} else {
				serUART->uart_open(ulWantedBaud);
				serUART->uart_control(UART_CMD_SET_BAUD, (void *)ulWantedBaud);
				serUART->uart_control(UART_CMD_SET_RXINT_BUF, NULL);
				serUART->uart_control(UART_CMD_SET_RXCB, vSerial_RXCallback);
				serUART->uart_control(UART_CMD_SET_TXINT_BUF, NULL);
				serUART->uart_control(UART_CMD_SET_TXCB, vSerial_TXCallback);
				serUART->uart_control(UART_CMD_SET_RXINT, (void *)1);
				serUART->uart_control(UART_CMD_SET_TXINT, (void *)0);
			}

			xReturn = ( xComPortHandle ) serUART;
		}
		portEXIT_CRITICAL();
	} else {
		xReturn = ( xComPortHandle ) 0;
